  ...
  describe("Contracts", async function () {
    it("Deploy contract", async function () {
      ...
      for (let companyIdx of [1, 2]) {
        ...
        console.log(
          `Company ${companyIdx} Owner account: ${companyOwnerAccount.address}`,
        );
        const { code: workerCode } =
          locklift.factory.getContractArtifacts("Worker");
        ...
        console.log(
          `Company ${companyIdx} address: ${companyContract.address}`,
        );

        let workers: Array<Worker> = [];
        // create workers
        for (let workerIdx of [1, 2]) {
          let { account: workerOwnerAccount } =
          ...
          console.log(
            `Company ${companyIdx} Worker ${workerIdx} account: ${workerOwnerAccount.address}`,
          );
          ...
          console.log(
            `Company ${companyIdx} Worker ${workerIdx} contract: ${workerContract.address}`,
          );
    ...
    it("Check details", async function () {
      for (let company of companies) {
        const companyDetails = await company.contract.methods
          .getDetails()
          .call();
        console.log(
          `Company ${company.contract.address} details: ${JSON.stringify(
            companyDetails,
            null,
            4,
          )}`,
        ...
        for (let worker of company.workers) {
          const details = await worker.workerContract.methods
            .getDetails()
            .call();
          console.log(
            `Company ${company.contract.address} Worker ${
              worker.workerContract.address
